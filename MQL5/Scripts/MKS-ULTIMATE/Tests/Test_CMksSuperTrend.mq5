//+------------------------------------------------------------------+
//| @file           : Test_CMksSuperTrend.mq5
//| @project        : MKS-ULTIMATE
//| @module         : Scripts / MKS-ULTIMATE / Tests
//| @responsibility : Valida CMksSuperTrend (versão sem ATR, flipping
...
//|                     3) Math: dado trend[i] vs trend[i-1], o active
//|                        stop bate com basicLower/basicUpper/ratchet
//|                     4) Flip rule: se trend mudou, close cruzou
//|                        active prévio na direção certa.
//|                   brickSize auto-inferido igual ao indicador.
//| @depends_on     : Indicators/MKS-ULTIMATE/CMksSuperTrend.mq5
//| @install_path   : MQL5/Scripts/MKS-ULTIMATE/Tests/Test_CMksSuperTrend.mq5
//+------------------------------------------------------------------+
#property strict
#property script_show_inputs

input double InpOffsetBricks   = 3.0;
input double InpBrickSizePts   = 0.0;
...
void OnStart()
{
   ...
   int bars  = MathMin(InpBarsToCheck, total - InpShift - 3);
   if(bars < 2)
   {
      PrintFormat("FAIL: bars insuficientes (total=%d, shift=%d)", total, InpShift);
   ...
   // Resolve brickSize igual ao indicador.
   ...
   else
   {
      ...
      if(sz <= 0.0)
      {
         Print("FAIL: auto-infer de brickSize falhou. Defina InpBrickSizePts.");
         IndicatorRelease(handle);
         return;
      }
      brickSizePrice = sz;
   ...
   for(int s = 0; s < bars - 1; s++)
   {
      ...
      // (3) Math: src é da bar fonte (bars-ago = s + srcLag)
      int srcBarsAgo = s + srcLag;
      double srcHi   = iHigh(_Symbol, _Period, srcBarsAgo);
      double srcLo   = iLow(_Symbol,  _Period, srcBarsAgo);
      double src     = (srcHi + srcLo) * 0.5;
      ...
      if(trendPrev == 0.0)
         ...
         expectedStop = MathMax(basicLower, activePrv);
      else if(trendNow < 0.0 && trendPrev < 0.0)
         expectedStop = MathMin(basicUpper, activePrv);
      else if(trendNow > 0.0 && trendPrev < 0.0)
         expectedStop = basicLower;
      else
         expectedStop = basicUpper;
   ...
   if(totalErrors == 0)
   {
      PrintFormat("OK: %d bars verificadas, zero divergencia (viz/eq/math/flip). "
                  "offset=%.2f brickSize(pts)=%.2f",
                  bars - 1, InpOffsetBricks, brickSizePrice / _Point);
   }
   else
   {
