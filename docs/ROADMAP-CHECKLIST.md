...
- [ ] **Slice-2: runner de stress liga/desliga** (estratégia sobre `SimulatedBroker` + `StressLabBroker` por níveis, comparando com/sem) — *critério: um relatório comparativo None→Nightmare de uma corrida.* → **PARTE 2 (prioridade Agora).**
...
- [ ] **Runner de estresse liga/desliga** — rodar a estratégia SEM o StressLab, depois COM, e comparar lado a lado. *Critério: um EA/script que produz um relatório comparativo (None→Nightmare) de uma corrida sobre `.mkstick`, com o net por nível.*
...
- [ ] **`flatten-on-breach` (breaker corretivo)** — fechar tudo + travar ao cruzar minEquity/drawdown em `OnTick` (hoje o breaker é só preventivo). *Critério: decidir construir ou não; se sim, componente + teste (ADR-036 §Fronteiras).*
