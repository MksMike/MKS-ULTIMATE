...
> **CORREÇÃO (2026-05-30) — o cap é necessário mas NÃO suficiente; o diagnóstico de causa-raiz desta emenda estava errado.** Re-anexar o EA com o build capado **continuou falhando** (inclusive durante o fill histórico). Análise multi-agente + forum MQL5 ([360938](https://www.mql5.com/en/forum/360938), [335324](https://www.mql5.com/en/forum/335324), [363224](https://www.mql5.com/en/forum/363224)) revelou que a causa-raiz é um **bug de plataforma não-corrigido do MT5**: `CustomRatesUpdate` corrompe o container do Custom Symbol na **virada de dia do server** (apaga a série inteira; `-1` com `GetLastError()=0`/4000; journal "invalid container header"/"1970.01.01"), **independente** de barra-no-futuro. A premissa desta emenda ("MT5 recusa barra além de ~fim-de-amanhã") era uma **inferência não-confirmada** de um único cruzamento observado — o limite não existe documentado. O cap permanece válido como **higiene** (remove um amplificador auto-infligido: bump levando barras dias ao futuro cruza mais fronteiras de dia), mas **não cura a morte do CS**. `CustomRatesReplace` e `CustomTicksAdd→OHLC` compartilham o mesmo container bugado. **A correção estrutural é aposentar o Custom Symbol do caminho de visualização** — renko brick-native (indicador `DRAW_COLOR_CANDLES`/objetos no chart do símbolo REAL do broker, que tem eixo de tempo real auto-mantido e sobrevive à meia-noite) + promover a família `IRenkoIndicator` da nota ADR-020-A. Decisão e detalhamento na **ADR-031** (em elaboração); ADR-020 §2/§4, ADR-021, ADR-023 e esta ADR-023-A passam a candidatas a *superseded* para o caminho de visualização live. ADR-020 §1 (estratégia nunca lê o visual) permanece e sai **fortalecida**.
>
> **REVERSÃO (2026-06-02) — o veredito de "bug incondicional/incorrigível" acima está REFUTADO; NÃO aposentar o CS.** A nota de 05-30 foi escrita sem comparar com o V5. O V5 (`reference/V5`) usava o **mesmo** `CustomRatesUpdate` e atravessava meia-noites de server **sem morrer** — logo a morte do CS não é um bug incondicional de plataforma. As causas reais são **auto-infligidas**: **(a)** o CS era criado **sem sessões 24/7** → o MT5 recusa `CustomRatesUpdate` na virada de dia (CAUSA-RAIZ; o V5 setava `CustomSymbolSetSessionQuote/Trade` 24/7 nos 7 dias); **(b)** os spec-setters (`SYMBOL_DIGITS/POINT/CHART_MODE/TICK_SIZE`) eram reaplicados a cada `OnInit`, apagando o histórico do CS a cada re-anexação do EA; **(c)** o sink não tinha recovery na falha. As frases "independente de barra-no-futuro" e "`CustomRatesReplace`/`CustomTicksAdd→OHLC` compartilham o mesmo container bugado" eram **conjectura sem fonte** — retiradas. Os threads de fórum descrevem uma corrupção de virada de dia **real mas CONDICIONAL** a **múltiplos** custom symbols cruzando a meia-noite (sem reprodução com CS único — nossa config); o 363224 suspeita de sessões só Seg-Sex. **Decisão: MANTER e CORRIGIR o CS.** ADR-020/021/023/023-A **não** são superseded — permanecem Aceitas; o cap da ADR-023-A fica como **higiene** (era band-aid sobre uma não-causa — o V5 derivava pro futuro sem cap e sobrevivia). **ADR-031 muda de "aposentar o CS" para "manter+corrigir o CS"** e ainda **não** é ADR escrita/aceita. Os 4 fixes aplicados (sessões 24/7; specs só na criação genuína; recovery no sink — re-select + 1 retry; marcador ancorado em `sink.lastBarTime`, não `iTime(CS,0)`) compilam 0/0. **GATE EMPÍRICO:** confirmado **0 `CS UPDATE FAIL`** em 2026-06-02 com vários restarts (confirma o fix (b), spec-wipe no re-attach); o cruzamento decisivo de meia-noite (06-02→06-03) com o build novo **ainda não ocorreu** → sobrevivência à virada de dia **pendente de dado** (observação→hipótese→**dado**→ADR).
...
- ~~**PENDENTE no MT5 (dono):** anexar o ColorReversal e conferir (a) o dialog nos 3 níveis; (b) um preset (XAU) sobrepõe brick/SL (log `starting` deve mostrar `preset:XAU, S:3.0, slBricks:10`); (c) Custom preserva o comportamento anterior; (d) um tester run abre/fecha ordens como antes.~~ **MT5-verificado (2026-07-21):** (a) dialog nos 3 níveis, sem K/L ✅; (b) preset XAU sobrepôs o input (`InpBrickSize=99` → log `"preset":"XAU","S":3.00000`, CS `MKSCR_3`) ✅; (c) Custom preservou (`"preset":"custom","S":99.00000`) ✅; OnInit limpo nos 3 runs. (d) tester order-cycle **não** re-exercitado — fora do diff da ADR-037 (order path intocado; já provado na Fase 9/E5).
...
2. **Latência→drift — MEDIDO e LIGADO.** Deslocamento líquido `|mid(t+L)−mid(t)|` (não a velocidade ingênua) dá **~0.5 pts/ms** (total) / **~0.25** (adverso) na janela realista ~260ms (bate com o roundtrip do demo da Fase 9, 524ms). O drift/ms cai com L (0.66→0.25 de 260ms→1000ms — random-walk √t), então o modelo linear `latencyDriftPointsPerMs` superestima em L alto (aceitável em Nightmare). **Ligado** via `latencyDriftPointsPerMs` no `MksStressRunnerConfig` + input `InpLatencyDriftPtsPerMs` do `StressReplayer`, **default 0.25 (adverso, tunável)** — decisão de modelagem = **expected-adverse** (metade move a favor), não worst-case. `slip = latencyMs · 0.25`, compõe com o `latencyMeanMs` do preset (Light 50ms≈12pts … Nightmare 1000ms≈250pts). Teste `Test_SR_LatencyDriftDegradesVsOff` (ligado vs desligado).
...
- **Dívida nomeada:** medir slippage de ordem real (requested vs filled) — só há 11 trades do demo, amostra insuficiente. Único eixo ainda não calibrado. (O drift de latência foi ligado neste mesmo ciclo, ver Decisão §2.)
...
- **Integrado no `ColorReversal.mq5`** (2026-07-22): input `InpEnableCorrectiveBreaker` (default ON), breaker envolve o gate preventivo (broker mais externo da estratégia), `breaker.OnTick()` dirigido por tick ANTES do `IngestOne` (age antes do Send). Muda o comportamento live: ao cruzar limite Por Conta, fecha + trava (antes só bloqueava a abertura). Desligar volta ao só-preventivo. Compila 52/0/0. **MT5-verificado (2026-07-22):** `Test_CMksCircuitBreaker` 18/18 (3 tests) + refactor SEM regressão (`Test_CMksRiskManager` 119/119, `Test_CMksRiskGatedBroker` 23/23). O smoke da integração no ColorReversal (ver o breaker disparar live) é demo opcional — exige depósito/lote que façam a perda cruzar o limite. Integração no stress runner fica opcional/futura.
