...
**Status:** Em andamento — **E2.0** (fundação DDR) completa e MT5-verificada; **E2.1 empiricamente VALIDADA (2026-07-21):** `verify-parity` **exit 0** sobre feed REAL (33.546 ticks XAUUSDm/Exness → 33 decisões / 17 flips, dois replays byte-a-byte idênticos). **E2.2 — golden bundle versionado (2026-07-21):** `tests/golden/e2-decision/` (fixture `.mkstick` + 2 journals golden + README); o **gate-crossing** foi provado no mesmo fixture (`InpMinEquityAbs=9990` → breaker 409 dispara; dois replays → exit 0, 18 decisões). O **determinismo da decisão — do brick ao breaker de conta — está provado sobre feed real.** **E2.4** (clock do feed) commitado. **E2.2 golden de BRICKS headless FEITO e MT5-verde (2026-07-23):** `Test_RealTickGolden` (Layer A — `CMksRenkoBuilder` real sobre o fixture via `CMksFileTickSource`, hermético, NÃO `CopyTicksRange`) compara os bricks campo-a-campo (incl. `triggerPrice`/`triggerTickId`/`high`/`low`) contra o golden `.mksbk` versionado (`tests/golden/e2-brick/`), falhando o TestRunner em divergência. **10/10 assertions, 0 failed.** Golden abençoado: 32 bricks, **17 flips == os 17 flips do golden da decisão já validado** → bricks corretos, não só auto-consistentes. Cumpre o 2º critério de saída ("teste headless que prova builder sobre ticks reais → bricks golden"). `Test_Producer` rebaixado a cross-check manual (não-hermético). **E2.3 FEITO e MT5-verde (2026-07-24 — `Test_CMksBrickFile` 110/110, `Test_CMksRenkoBuilder` 503/503):** âncora de proveniência (`seedMid`+`seedTickSeq`) no header do `.mksbk` (offset 192/200, **forward-compat** sobre espaço reservado, `formatVersion` segue 1 → arquivos pré-E2.3 leem 0/0 = ausente; patchada no `Close`/`Checkpoint`; getters no builder+reader; `HasSeed`) + **fail-fast** `InpParityRunMode` ⇒ exige `InpHistoricalFillDays=0` no Producer. **Verificado adversarialmente** (3 lentes, todas `safe`+0 regressões: backward-compat, paridade Producer↔Replayer idêntica por construção, `verify-parity` fortalecido — a seed pega divergência de origem). +3 testes (round-trip da seed, seed-ausente, captura no builder); nota na ADR-024. Diferido (só relevante em `InpSizerMode=ATR`): snapshot do sizer no header. **E2.4 VERIFICADO e MT5-verde (2026-07-24 — `Test_CMksDecisionRunner` 45/45):** `Test_Runner_MidnightRolloverDeterministic` prova que a fronteira de dia UTC do snapshot é **feed-driven** (`CMksFeedClock`, não wall-clock) — um stream cruzando 00:00 UTC (feed-time 2026-05-23, ≠ wall-clock 2026-07-24: se fosse `TimeCurrent()` o rollover NÃO dispararia e o teste falharia) rola o baseline no feed-midnight, e dois runs são **byte-idênticos na virada**; getter `DayStartMsc()` no runner. O clock do feed já estava **simétrico nos dois lados** (ColorReversal live + DecisionRunner replay ambos usam `CMksFeedClock.SetNow(t.timeMsc)`). **Golden de DECISÃO headless FEITO e MT5-verde (2026-07-24 — `Test_DecisionGolden` 5/5, casou de 1ª):** `Test_DecisionGolden` roda o `CMksDecisionRunner` sobre o fixture (config A = defaults + `slPoints=30000`) e compara o journal produzido vs `baseline.golden.tsv` **ignorando as linhas `#`** (mesma normalização do `verify-parity`), falhando o `TestRunner` em divergência — a paridade de decisão deixa de ser procedimento manual e vira rede de regressão. **Config B (gate-crossing) automatizada (2026-07-24, MT5-verde pendente):** `Test_DecisionGolden_MatchesGate` (`minEquityAbs=9990`) compara vs `gate-minequity.golden.tsv` (1 trade FILLED + 16 SEND REJECTED 409). Robustez: a partir do trip do gate todo SEND é rejeitado, logo o journal é determinístico independente do `tickValue` exato — só o 1º trade precisa disparar o gate (perda ~30 USD > 10 com `tickValue=1.0`). **Cadência/baseline do snapshot — RESOLVIDO (2026-07-24, decisão):** o **núcleo** do achado (`parity-mt5clock-wallclock-leak` + `parity-mt5clock-isready-asymmetry`, ambos rooted no `CMksMt5Clock`) foi **resolvido pelo E2.4** — live (`ColorReversal`) e replay (`CMksDecisionRunner`) usam **ambos** `CMksFeedClock` (grep confirma: `CMksMt5Clock` fora de todo composition root de decisão, só sobra no seu próprio smoke test). **Residual aceito e documentado:** o live seedeia o baseline do último tick **pré-attach** (`SymbolInfoTick`) + `Init()` no `OnInit`; o replay faz `Init()` **lazy** no 1º tick do `.mkstick` — diferem só no tick exato do baseline, **mesmo dia UTC no attach** (attach e 1º tick a segundos), **auto-corrige** via 1º rollover se cruzarem a meia-noite. A paridade **sim↔sim** automatizada (`Test_DecisionGolden`, ambos lazy) é bit-exata; live↔replay é estruturalmente não-bit-exato de qualquer forma (fills do broker real). Endurecimento opcional (tornar o `ColorReversal` lazy-init pra simetria total do baseline) **diferido** — toca o caminho live sem ganho de paridade hoje. **→ E2 FECHADO.** (Demo `DecisionReplayer` sobre `.mkstick` cruzando a meia-noite: opcional, precisa de fixture de virada.)
**Depende de:** E1.
